...
async function cleanupSession(sessionId: string) {
    ...
    console.log(
        `Session ${sessionId} cleaned up. Active sessions: ${sessions.size}`,
    );
}
    ...
    try {
        ...
        stream.on("data", (data: Buffer) => {
            if (ws.readyState === WebSocket.OPEN) {
                ...
                // Buffer data if WebSocket is not open
        ...
        console.log(
            `Session ${sessionId} started. Active sessions: ${sessions.size}`,
        );
    } catch (err) {
